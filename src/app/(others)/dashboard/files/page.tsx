...
const DashboardPosts = () => {
  ...
  // const [files, setFiles] = useState(files); // State to manage the list of files
  ...
  const handleViewFile = (file) => {
    ...
    switch (file.type.toUpperCase()) {
      case "PDF":
        window.open('http://localhost:3003/public/uploads/c567bb48-c045-41a1-9a62-07002256116f.pdf', "_blank"); // Opens in browser PDF viewer
        break;
        
      case "EPUB":
        window.open(`https://www.epubreader.xyz/?file=${encodeURIComponent(path.join("http://localhost:3003", file.path))}`, "_blank"); 
        break;
        
      case "MP3":
        setSelectedFile(file);
        setIsModalOpen(true); // Opens modal with audio player
        break;
        
      case "TXT":
        window.open(path.join("http://localhost:3003", file.path), "_blank"); // Opens text in a new tab
        break;
        
      default:
        alert("Format non pris en charge pour la lecture.");
    }
  };

  const handleDownload = (file) => {
  // Construct the URL for the GET request, using the correct path parameter
  const downloadUrl = `http://localhost:3003/api/download/${file.id}`;
  ...
  return (
    <div className="flex flex-col space-y-8">
          ...
          <div className="shadow dark:border dark:border-neutral-800 overflow-hidden sm:rounded-lg">
            <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-800">
              ...
              <tbody className="bg-white dark:bg-neutral-900 divide-y divide-neutral-200 dark:divide-neutral-800">
                {files.map((file) => (
                  <tr key={file.id}  className="cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-800">
                    <td className="px-6 py-4">
                      <div className="flex items-center w-96 lg:w-auto max-w-md overflow-hidden">
                        <NcImage onClick={() => handleRowClick(file)}
                    ...
                    <a
                      ...
                      onClick={(e) => {
                        ...
                        if (file.isDownloadable) {
                          handleDownload(file); // Call the handleDownload function when clicked
                        }
                      }}
                    >
                      Télécharger
      ...
      {isModalOpen && selectedFile && <FileDetailsModal file={selectedFile} onClose={() => setIsModalOpen(false)} toggleFavorite={toggleFavorite} />}
    </div>
  );
};
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-lg w-full max-w-lg relative">
        ...
        <div className="mt-6 flex gap-3">
          ...
          <button
            onClick={() => toggleFavorite(file.id)}  // This toggles the favorite status
